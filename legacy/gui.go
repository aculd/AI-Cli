...
type ChatModel struct {
	...
	focused            bool      // Add 'focused' field
	appState           *AppState // Pointer to global app state (for modal manager)
}
...
func (m ChatModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// If a modal is active, route all input to the modal
	if m.appState != nil && m.appState.ModalManager != nil && m.appState.ModalManager.Current() != nil {
		modal := m.appState.ModalManager.Current()
		updated, cmd := modal.Update(msg)
		// If modal was closed, pop it
		if updated == nil {
			m.appState.ModalManager.Pop()
		}
		return m, cmd
	}
	switch msg := msg.(type) {
	case tea.KeyMsg:
		...
		if msg.String() == "q" {
			// Push ConfirmationModal onto ModalManager
			modal := dialogs.NewConfirmationModal(
				"Are you sure you want to quit?",
				[]modals.ModalOption{
					{Label: "Yes", OnSelect: func() { m.quitting = true }},
					{Label: "No", OnSelect: func() {}},
				},
				func() { m.appState.ModalManager.Pop() },
			)
			m.appState.ModalManager.Push(*modal)
			return m, nil
		}
		...
		case "home":
			if m.focused && !m.loading && m.inputBuffer == "" {
				// m.goToTop()
				m.autoScroll = false
			} else if m.focused {
				m.cursorPos = 0
		...
		case "enter":
			if m.inputBuffer != "" && !m.loading {
				if strings.HasPrefix(m.inputBuffer, ":") {
					if g.handleVimCommand(m.inputBuffer) {
						if m.generatingTitle {
							m.inputBuffer = ""
		...
		case "ctrl+o":
			if m.selectedMessageIdx >= 0 {
				msg := m.getPageMessages()[m.selectedMessageIdx]
				modal := input.NewEditorModal(msg.Content)
				m.appState.ModalManager.Push(modal)
			}
		case "f1":
			m.showHelp = true
...
func (m ChatModel) View() string {
	...
	if m.showHelp {
		// Render the InformationModal with controls cheat sheet
		helpContent := `
...
(Press Esc to close)`
	...
	if m.inputBuffer == "" {
		...
		for i := 0; i <= len(inputRunes); i++ {
			isCursor := (i == m.cursorPos)
			isSelected := m.selectStart != -1 && m.selectEnd != -1 && i > min(m.selectStart, m.selectEnd)-1 && i <= max(m.selectStart, m.selectEnd)
			var ch string
			if i < len(inputRunes) {
				ch = string(inputRunes[i])
			} else if isCursor {
				ch = "_" // underscore at end
			}
			if isCursor && i < len(inputRunes) && !isSelected {
				// Underline the character at the cursor if not selected
				ch = lipgloss.NewStyle().Underline(true).Render(ch)
			}
			if isSelected {
				ch = lipgloss.NewStyle().Background(lipgloss.Color("21")).Foreground(lipgloss.Color("15")).Render(ch)
			}
			if isCursor && i == len(inputRunes) {
				ch = "_" // underscore at end
			}
			renderedInput += ch
		}
...
func (m MenuModel) View() string {
	...
	controlsText := "Controls: ↑↓ to navigate, Enter to select, Esc to go back, Ctrl+Q to quit"
	controls := lipgloss.NewStyle().Align(lipgloss.Center).Render(controlsText)

	if m.title == "Main Menu" {
		menuBlock := lipgloss.JoinVertical(lipgloss.Center, asciiArt, title, centeredMenu, controls)
		centeredMenuBlock := lipgloss.NewStyle().Width(w).Height(h).Align(lipgloss.Center, lipgloss.Center).Render(menuBlock)
		return centeredMenuBlock
	} else if title != "" {
		menuBlock := lipgloss.JoinVertical(lipgloss.Center, title, centeredMenu, controls)
		centeredMenuBlock := lipgloss.NewStyle().Width(w).Height(h).Align(lipgloss.Center, lipgloss.Center).Render(menuBlock)
		return centeredMenuBlock
	} else {
		menuBlock := lipgloss.JoinVertical(lipgloss.Center, centeredMenu, controls)
		centeredMenuBlock := lipgloss.NewStyle().Width(w).Height(h).Align(lipgloss.Center, lipgloss.Center).Render(menuBlock)
		return centeredMenuBlock
	}
}
...
func GUIMenuAPIKey() error {
	apiKeyMenuOptions := []string{"List API Keys", "Add API Key", "Remove API Key", "Set Active API Key", "Test Active Key", "Back"}
	for {
		model := MenuModel{
			title:    "API Key",
		...
		case 4: // Test Active Key
			models, _, err := loadModelsWithMostRecent()
			if err != nil || len(models) == 0 {
				ShowErrorModal("No models available to test.")
				continue
			}
			modelIdx, err := selectFromList("Select Model to Test", models)
			if err != nil || modelIdx < 0 || modelIdx >= len(models) {
				continue
			}
			modelName := models[modelIdx]
			result := TestAPIKeyWithModel(modelName)
			infoModal := InformationModal{
				Title:   "Test API Key Result",
				Content: result,
				Width:   60,
				Height:  8,
			}
...
func (m YesNoModel) View() string {
	...
	box := lipgloss.NewStyle().
		...
		Render(content)
	return lipgloss.Place(boxWidth, 24, lipgloss.Center, lipgloss.Center, box)
}
...
type AppState struct {
	openChats     map[string]*ChatModel // chat name to model
	openChatOrder []string              // order of open chats
	activeChatIdx int                   // index in openChatOrder
	inChatList    bool                  // true if in chat list menu
	menuTabOpen   bool                  // true if menu tab is open
	menuTabIndex  int                   // selection index for menu tab
	ModalManager  *modals.ModalManager  // manages the stack of active modals (confirmation, help, input, etc.)
}

// NewAppState creates a new AppState with initialized ModalManager.
func NewAppState() *AppState {
	return &AppState{
		openChats:     make(map[string]*ChatModel),
		openChatOrder: []string{},
		activeChatIdx: 0,
		inChatList:    false,
		menuTabOpen:   false,
		menuTabIndex:  0,
...
// MultiChatModel manages multiple open chats and the multi-pane UI
// Sidebar: open chats, Title bar: model+count+status, Main: chat, Bottom: input
// Ctrl+Tab cycles active chat
...
func NewMultiChatModel(appState *AppState, width, height int) MultiChatModel {
...
func (m MultiChatModel) Init() tea.Cmd {
	return nil
}

func (m MultiChatModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyMsg:
		if msg.String() == "ctrl+t" {
			// Open menu tab if not already open
			if !m.appState.menuTabOpen {
				m.appState.openChatOrder = append(m.appState.openChatOrder, menuTabName)
				m.appState.menuTabOpen = true
				m.appState.activeChatIdx = len(m.appState.openChatOrder) - 1
			} else {
				// Switch to menu tab
				for i, name := range m.appState.openChatOrder {
					if name == menuTabName {
						m.appState.activeChatIdx = i
						break
					}
				}
			}
		...
		if msg.String() == "ctrl+w" {
			// Close current tab
			if len(m.appState.openChatOrder) == 1 {
				// If only one tab, show menu tab
				if m.appState.openChatOrder[0] != menuTabName {
					m.appState.openChatOrder = []string{menuTabName}
					m.appState.menuTabOpen = true
					m.appState.activeChatIdx = 0
				} // else do nothing
			} else {
				idx := m.appState.activeChatIdx
				name := m.appState.openChatOrder[idx]
				if name == menuTabName {
					m.appState.menuTabOpen = false
				}
				if _, ok := m.appState.openChats[name]; ok {
					delete(m.appState.openChats, name)
				}
				m.appState.openChatOrder = append(m.appState.openChatOrder[:idx], m.appState.openChatOrder[idx+1:]...)
				if idx >= len(m.appState.openChatOrder) {
					m.appState.activeChatIdx = len(m.appState.openChatOrder) - 1
				} else {
					m.appState.activeChatIdx = idx
				}
			}
			return m, nil
		}
		if msg.String() == "ctrl+tab" {
			if len(m.appState.openChatOrder) > 1 {
				m.appState.activeChatIdx = (m.appState.activeChatIdx + 1) % len(m.appState.openChatOrder)
			}
			return m, nil
		}
		// Forward to active chat if not menu tab
		if m.activeTabName() != menuTabName {
			active := m.activeChatModel()
			updated, cmd := active.Update(msg)
			m.setActiveChatModel(updated)
			return m, cmd
		}
		// Menu tab: handle selection (simulate up/down/enter)
...
func (m *MultiChatModel) activeTabName() string {
	if len(m.appState.openChatOrder) == 0 {
		return ""
	}
	return m.appState.openChatOrder[m.appState.activeChatIdx]
}

func (m MultiChatModel) View() string {
	if len(m.appState.openChatOrder) == 0 {
		return "No open tabs."
	}
	sidebarWidth := 24
	...
	mainHeight := h - inputHeight - titleHeight - 2
	if mainWidth < 20 {
		mainWidth = 20
	}
	if mainHeight < 5 {
	...
	// Sidebar: list open tabs
	sidebar := ""
	for i, name := range m.appState.openChatOrder {
		style := lipgloss.NewStyle().Foreground(lipgloss.Color("245"))
		if i == m.appState.activeChatIdx {
			style = style.Bold(true).Foreground(lipgloss.Color("33"))
		}
		line := style.Render(name)
		sidebar += line + "\n"
	}
	sidebarBox := lipgloss.NewStyle().Width(sidebarWidth).Border(lipgloss.NormalBorder()).Render(sidebar)

	if m.activeTabName() == menuTabName {
		chats, _ := listChats()
		menu := "List of chats:\n"
		for i, c := range chats {
			selected := (i == m.appState.menuTabIndex)
		...
		selected := (m.appState.menuTabIndex == len(chats))
		style := lipgloss.NewStyle()
		if selected {
			style = style.Bold(true).Foreground(lipgloss.Color("33")).Background(lipgloss.Color("236"))
		}
		menu += style.Render("  + Create New Chat") + "\n"
		menu += "\n[Enter] Open chat  [N] New chat  [Esc] Close menu tab"
		mainArea := lipgloss.NewStyle().Width(mainWidth).Height(mainHeight).Border(lipgloss.NormalBorder()).Render(menu)
		// Title bar: model: - | status: creating new tab
		blue := lipgloss.NewStyle().Foreground(lipgloss.Color("33")).Bold(true)
		title := blue.Render("model: -") + lipgloss.NewStyle().Width(mainWidth-20).Render("") + blue.Render("status: creating new tab")
		titleBar := lipgloss.NewStyle().Width(mainWidth).Render(title)
		layout := lipgloss.JoinHorizontal(lipgloss.Top, sidebarBox, lipgloss.JoinVertical(lipgloss.Left, titleBar, mainArea))
		return lipgloss.Place(m.width, m.height, lipgloss.Center, lipgloss.Center, layout)
	}

	// ... existing chat rendering for active chat ...

	// Active chat
	active := m.activeChatModel()

	// Title bar: model name, num messages, status (blue)
	modelName := active.model
	msgCount := len(active.messages)
	status := active.status
	blue := lipgloss.NewStyle().Foreground(lipgloss.Color("33")).Bold(true)
	title := blue.Render(fmt.Sprintf("%s | %d msgs | %s", modelName, msgCount, status))
	titleBar := lipgloss.NewStyle().Width(mainWidth).Render(title)

	// Chat window (scrollable)
	active.width = mainWidth
	active.height = mainHeight
	chatView := active.View()

	// Input box (reuse existing rendering)
	active.width = mainWidth
	active.height = inputHeight
	inputBox := ""
	if active.inputBuffer == "" {
		inputBox = lipgloss.NewStyle().Width(mainWidth).Height(inputHeight).Border(lipgloss.NormalBorder()).Render("Input: ")
	} else {
		inputBox = lipgloss.NewStyle().Width(mainWidth).Height(inputHeight).Border(lipgloss.NormalBorder()).Render("Input: " + active.inputBuffer)
	}

	// Compose main area
	mainArea := lipgloss.JoinVertical(lipgloss.Left, titleBar, chatView, inputBox)

	// Compose full layout
	layout := lipgloss.JoinHorizontal(lipgloss.Top, sidebarBox, mainArea)
	return lipgloss.Place(m.width, m.height, lipgloss.Center, lipgloss.Center, layout)
}

// Replace RunGUIMultiChat with new MultiChatModel-based loop
func RunGUIMultiChat() error {
	app := &AppState{
		openChats:     make(map[string]*ChatModel),
		openChatOrder: []string{},
		activeChatIdx: 0,
	...
	model.autoScroll = true
	app.openChats[chatName] = &model
	app.openChatOrder = append(app.openChatOrder, chatName)
	app.activeChatIdx = 0

	multi := NewMultiChatModel(app, 120, 40)
	p := tea.NewProgram(multi, tea.WithAltScreen(), tea.WithMouseCellMotion())
	_, err = p.Run()
	return err
}

// Add at the top of the file, after imports:
const menuTabName = "[ + New Tab ]"

// Re-add these methods to MultiChatModel:
func (m *MultiChatModel) activeChatModel() *ChatModel {
	if len(m.appState.openChatOrder) == 0 {
		return nil
	}
	name := m.appState.openChatOrder[m.appState.activeChatIdx]
	return m.appState.openChats[name]
}

func (m *MultiChatModel) setActiveChatModel(model tea.Model) {
	if len(m.appState.openChatOrder) == 0 {
		return
	}
	name := m.appState.openChatOrder[m.appState.activeChatIdx]
	if chat, ok := model.(ChatModel); ok {
		m.appState.openChats[name] = &chat
	}
}
